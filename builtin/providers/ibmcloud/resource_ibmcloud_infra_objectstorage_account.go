...
func resourceIBMCloudInfraObjectStorageAccount() *schema.Resource {
	return &schema.Resource{
		Create:   resourceIBMCloudInfraObjectStorageAccountCreate,
		Read:     resourceIBMCloudInfraObjectStorageAccountRead,
		Update:   resourceIBMCloudInfraObjectStorageAccountUpdate,
		Delete:   resourceIBMCloudInfraObjectStorageAccountDelete,
		Exists:   resourceIBMCloudInfraObjectStorageAccountExists,
...
func resourceIBMCloudInfraObjectStorageAccountCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("resource_ibmcloud_infra_objectstorage_account: Error on create: %s", err)
	}
	...
	if len(objectStorageAccounts) == 0 {
		...
		if err != nil {
			return fmt.Errorf(
				"resource_ibmcloud_infra_objectstorage_account: Error ordering account: %s", err)
		}
		...
		if err != nil {
			return fmt.Errorf("resource_ibmcloud_infra_objectstorage_account: Error on retrieving new: %s", err)
		}

		if len(objectStorageAccounts) == 0 {
			return fmt.Errorf("resource_ibmcloud_infra_objectstorage_account: Failed to create object storage account.")
		}
	}
...
func resourceIBMCloudInfraObjectStorageAccountRead(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	accountService := services.GetAccountService(sess)
	accountName := d.Id()
	...
	if err != nil {
		return fmt.Errorf("resource_ibmcloud_infra_objectstorage_account: Error on Read: %s", err)
	}
	...
	return fmt.Errorf("resource_ibmcloud_infra_objectstorage_account: Could not find account %s", accountName)
}

func resourceIBMCloudInfraObjectStorageAccountUpdate(d *schema.ResourceData, meta interface{}) error {
	// Nothing to update for now. Not supported.
	return nil
}

func resourceIBMCloudInfraObjectStorageAccountDelete(d *schema.ResourceData, meta interface{}) error {
