...
var IBMCloudInfraScaleGroupObjectMask = []string{
	"id",
	"name",
	"minimumMemberCount",
	"maximumMemberCount",
...
func resourceIBMCloudInfraScaleGroup() *schema.Resource {
	return &schema.Resource{
		Create:   resourceIBMCloudInfraGroupCreate,
		Read:     resourceIBMCloudInfraScaleGroupRead,
		Update:   resourceIBMCloudInfraScaleGroupUpdate,
		Delete:   resourceIBMCloudInfraScaleGroupDelete,
		Exists:   resourceIBMCloudInfraScaleGroupExists,
...
func getModifiedVirtualGuestResource() *schema.Resource {
	...
	r := resourceIBMCloudInfraVirtualGuest()
...
func getVirtualGuestTemplate(vGuestTemplateList []interface{}, meta interface{}) (datatypes.Virtual_Guest, error) {
	...
	// Create an empty ResourceData instance for a IBMCloudInfra_Virtual_Guest resource
	vGuestResourceData := resourceIBMCloudInfraVirtualGuest().Data(nil)
...
func resourceIBMCloudInfraGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceIBMCloudInfraScaleGroupRead(d, meta)
}
...
func resourceIBMCloudInfraScaleGroupRead(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetScaleGroupService(sess)

	groupId, _ := strconv.Atoi(d.Id())

	slGroupObj, err := service.Id(groupId).Mask(strings.Join(IBMCloudInfraScaleGroupObjectMask, ",")).GetObject()
	if err != nil {
		...
		return fmt.Errorf("Error retrieving infrastructure Scale Group: %s", err)
	}
...
func resourceIBMCloudInfraScaleGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	...
	groupObj, err := scaleGroupService.Id(groupId).Mask(strings.Join(IBMCloudInfraScaleGroupObjectMask, ",")).GetObject()
	if err != nil {
		return fmt.Errorf("Error retrieving infrastructure_scale_group resource: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error received while editing infrastructure_scale_group: %s", err)
	}
...
func resourceIBMCloudInfraScaleGroupDelete(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	scaleGroupService := services.GetScaleGroupService(sess)
...
func resourceIBMCloudInfraScaleGroupExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	sess := meta.(ClientSession).SoftLayerSession()
	scaleGroupService := services.GetScaleGroupService(sess)
