...
const (
	IBMCloudInfraTimeFormat                                 = string("2006-01-02T15:04:05-07:00")
	IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_RESOURCE_USE = 1
	IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_REPEATING    = 2
	IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_ONE_TIME     = 3
)

var IBMCloudInfraScalePolicyObjectMask = []string{
	"cooldown",
	"id",
	"name",
	"scaleActions",
...
func resourceIBMCloudInfraScalePolicy() *schema.Resource {
	return &schema.Resource{
		Create:   resourceIBMCloudInfraScalePolicyCreate,
		Read:     resourceIBMCloudInfraScalePolicyRead,
		Update:   resourceIBMCloudInfraScalePolicyUpdate,
		Delete:   resourceIBMCloudInfraScalePolicyDelete,
		Exists:   resourceIBMCloudInfraScalePolicyExists,
		...
		Schema: map[string]*schema.Schema{
			...
			"triggers": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"watches": {
							...
							Set: resourceIBMCloudInfraScalePolicyHandlerHash,
						},
				...
				Set: resourceIBMCloudInfraScalePolicyTriggerHash,
			},
		},
	}
}

func resourceIBMCloudInfraScalePolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceIBMCloudInfraScalePolicyRead(d, meta)
}

func resourceIBMCloudInfraScalePolicyRead(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetScalePolicyService(sess)
	...
	scalePolicy, err := service.Id(scalePolicyId).Mask(strings.Join(IBMCloudInfraScalePolicyObjectMask, ";")).GetObject()
	if err != nil {
...
func resourceIBMCloudInfraScalePolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	...
	scalePolicy, err := scalePolicyService.Id(scalePolicyId).Mask(strings.Join(IBMCloudInfraScalePolicyObjectMask, ";")).GetObject()
	if err != nil {
		return fmt.Errorf("Error retrieving scalePolicy: %s", err)
	}
...
func resourceIBMCloudInfraScalePolicyDelete(d *schema.ResourceData, meta interface{}) error {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetScalePolicyService(sess)
...
func resourceIBMCloudInfraScalePolicyExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	sess := meta.(ClientSession).SoftLayerSession()
	service := services.GetScalePolicyService(sess)
...
func prepareOneTimeTriggers(d *schema.ResourceData) ([]datatypes.Scale_Policy_Trigger_OneTime, error) {
	...
	for _, triggerList := range triggerLists {
		...
		if trigger["type"].(string) == "ONE_TIME" {
			...
			oneTimeTrigger.TypeId = sl.Int(IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_ONE_TIME)
			...
			timeStamp, err := time.Parse(IBMCloudInfraTimeFormat, timeStampString)
			if err != nil {
				return nil, err
			}
...
func prepareRepeatingTriggers(d *schema.ResourceData) ([]datatypes.Scale_Policy_Trigger_Repeating, error) {
	...
	for _, triggerList := range triggerLists {
		...
		if trigger["type"].(string) == "REPEATING" {
			...
			repeatingTrigger.TypeId = sl.Int(IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_REPEATING)
...
func prepareResourceUseTriggers(d *schema.ResourceData) ([]datatypes.Scale_Policy_Trigger_ResourceUse, error) {
	...
	for _, triggerList := range triggerLists {
		...
		if trigger["type"].(string) == "RESOURCE_USE" {
			...
			var err error
			resourceUseTrigger.TypeId = sl.Int(IBMCLOUDINFRA_SCALE_POLICY_TRIGGER_TYPE_ID_RESOURCE_USE)
...
func readOneTimeTriggers(list []datatypes.Scale_Policy_Trigger_OneTime) []map[string]interface{} {
	...
	for _, trigger := range list {
		...
		t["id"] = *trigger.Id
		t["type"] = "ONE_TIME"
		t["date"] = trigger.Date.In(UTCZone).Format(IBMCloudInfraTimeFormat)
...
func readResourceUseTriggers(list []datatypes.Scale_Policy_Trigger_ResourceUse) []map[string]interface{} {
	...
	for _, trigger := range list {
		...
		t["id"] = *trigger.Id
		t["type"] = "RESOURCE_USE"
		t["watches"] = schema.NewSet(resourceIBMCloudInfraScalePolicyHandlerHash,
...
func resourceIBMCloudInfraScalePolicyTriggerHash(v interface{}) int {
...
func resourceIBMCloudInfraScalePolicyHandlerHash(v interface{}) int {
	var buf bytes.Buffer
